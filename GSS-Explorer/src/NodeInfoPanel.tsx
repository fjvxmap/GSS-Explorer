...
export function NodeInfoPanel({ node, solutions }: NodeInfoPanelProps) {
  if (!node) {
    return (
      <div style={{
        ...
        <p style={{ color: '#666', fontSize: '13px' }}>Click on a node to see details</p>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      <h3 style={{ marginTop: 0, fontSize: '16px', marginBottom: '10px', flexShrink: 0 }}>Node Information</h3>
      ...
      <div style={{
        ...
        <span style={{ fontFamily: 'monospace' }}>{node.current_clique || 'empty'}</span>
      ...
      {solutions.length > 0 && (
        <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <h4 style={{ marginBottom: '6px', fontSize: '14px', flexShrink: 0 }}>Solutions Found ({solutions.length}):</h4>
          <div style={{
            flex: 1,
            overflowY: 'auto',
            fontSize: '11px',
